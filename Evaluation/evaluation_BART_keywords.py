...
with open('output_BART_keywords.csv', newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
        references.append(row[2].split())
        hypothesis.append(row[4].split())
...
print("Rouge score is: "+str(score_rouge["rougeL"].mid[2]*100 ))   #fmeasure
#score_rouge.keys()
...
print("METEOR score is: "+str(score_meteor['meteor']*100))
...
data = pd.read_csv('output_BART_keywords.csv')
...
for i in range(1, 5):
    print(f"BLEU-{i} score is: {scores[i-1] * 100:.2f}")