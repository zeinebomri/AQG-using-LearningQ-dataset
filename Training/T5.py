...
tokenizer.sep_token = '<sep>'   #we are telling the tokenizer to use <sep> as the separator token
tokenizer.add_tokens(['<sep>'])  #By adding '<sep>' to the tokenizer's vocabulary, we ensure that the tokenizer can correctly encode and decode the <sep> token during tokenization and generation
...
torch.save(train_dataset, './results_t5/train_data.pt')
torch.save(valid_dataset, './results_t5/valid_data.pt')
...
for params in param_combinations:
    ...
    if valid_loss < best_valid_loss:
        ...
        model.save_pretrained('checkpoint_T5')
